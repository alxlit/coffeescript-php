...
indexValue(A) ::= expression(B)   . { A = new yyIndex(B); }
indexValue(A) ::= slice(B)        . { A = new yySlice(B); }

object(A) ::= YY_OBJECT_START assignList(B) optComma(C) YY_OBJECT_END   . { A = new yyObj(C, isset(B['generated']) ? B['generated'] : FALSE); }
...
this(A) ::= YY_AT_SIGN  . { A = new yyValue(new yyLiteral('this')); }

thisProperty(A) ::= YY_AT_SIGN identifier(B)  . { A = new yyValue(new yyLiteral('this'), array(new yyAccess(B)), 'this'); }
...
slice(A) ::= expression(B) rangeDots(C) expression(D)   . { A = new yyRange(B, D, C); }
